...
export function removerCartItem(productId) {
  const request = axios.get(`/api/users/removerFromCart?id=${productId}`).then((response) => {
    //productInfo, cart 정보를 조합해서 CartDetail을 만든다.
    response.data.cart.forEach((item) => {
      response.data.productInfo.forEach((product, index) => {
        if (item.id === product._id) {
          response.data.productInfo[index].quantitiy = item.quantity;
        }
      });
    });
    return response.data;
